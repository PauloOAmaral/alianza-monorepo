...
import { createContext, createCookieSessionStorage, href, type MiddlewareFunction, type RouterContextProvider, redirect, type Session } from 'react-router'
...
async function getUserSession(request: Request): Promise<UserSession | null> {
    const cookieSession = await getCookieSession(request)
    const sessionId = cookieSession.get('id')

    if (!sessionId) return null

    const session = await getDatabaseSession(createRequest(request, { sessionId })).catch(() => null)

    return session?.data ?? null
}

function shouldDestroySession(prev: Partial<CookieSession> | null | undefined, current: Partial<CookieSession> | null | undefined): boolean {
    return Boolean(prev?.id && !current?.id)
}

function shouldCommitSession(prev: Partial<CookieSession> | null | undefined, current: Partial<CookieSession> | null | undefined): boolean {
...
async function requireSession(request: Request): Promise<UserSession> {
    ...
    headers.append('Set-Cookie', await sessionStorage.commitSession(await getCookieSession(request)))

    throw redirect(href('/login'), { headers })
}

async function createSession(request: Request, id: string): Promise<Headers> {
    const cookieSession = await getCookieSession(request)
...
async function removeSession(request: Request): Promise<Headers> {
    ...
    const headers = new Headers()
    headers.append('Set-Cookie', await sessionStorage.destroySession(cookieSession))

    return headers
}

const sessionMiddleware: MiddlewareFunction<Response> = async ({ request }, next) => {
    ...
    if (shouldDestroySession(initialData, currentData)) {
        response.headers.append('Set-Cookie', await sessionStorage.destroySession(cookieSession))
    } else if (shouldCommitSession(initialData, currentData)) {
        response.headers.append('Set-Cookie', await sessionStorage.commitSession(cookieSession))
    }

    return response
}

export { sessionMiddleware, requireSession, createSession, removeSession, getCookieSession, getUserSession }
