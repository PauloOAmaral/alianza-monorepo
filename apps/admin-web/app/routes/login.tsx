...
export function meta() {
    return [
        { title: "Login Alianza" },
        { name: "description", content: "Login to your Alianza account" },
    ]
}
...
export async function action({ request }: Route.ActionArgs) {
    ...
    const { success, value, errors } = await parseFormDataWithZod(
        formData,
        loginWithPasswordSchema(t),
    )

    if (!success) {
        const message = Array.isArray(errors)
            ? errors.join(". ")
            : (errors ?? t("serverError.unexpected"))

        return await dataWithError({ success: false, message }, errors ?? [], { status: 400 })
    }

    const { success: turnstileSuccess } = await validateTurnstile(
        value["cf-turnstile-response"] as string,
    )

    if (!turnstileSuccess) {
        return data(
            { success: false, message: t("errors.invalidCaptchaResponse") },
            { status: 400 },
        )
    }
