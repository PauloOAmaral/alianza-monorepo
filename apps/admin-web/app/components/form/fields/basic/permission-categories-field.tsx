import { Accordion, AccordionContent, AccordionHeading, AccordionItem } from '@alianza/ui/accordion'
...
export function PermissionCategoriesField({ name }: PermissionCategoriesFieldProps) {
    ...
    function getSelectedPermissionsCount(categoryPermissions: Permission[]) {
        ...
        const selectedCount = categoryPermissions.filter(permission => permissions.includes(permission.name)).length
                ...
                render={() => (
                    <FormItem className='flex w-full gap-2 min-w-0 flex-row p-2'>
                        <div className='flex-1'>
                            ...
                            <p className='text-sm text-muted-foreground'>{t('permissions.permissionFullDescription')}</p>
                        </div>
                        <FormControl>
                            <Switch aria-describedby='permissions-error' isSelected={fullPermissionSelected} onChange={value => handlePermissionChange('full', value)} />
                        </FormControl>
                    </FormItem>
                )}
            />
                                            ...
                                            <FormItem className='flex flex-row items-center justify-between'>
                                                ...
                                                <FormControl>
                                                    <Switch
                                                        ...
                                                        isSelected={fullPermissionSelected || permissions.includes(permission.name)}
                                                        onChange={value => handlePermissionChange(permission.name, value)}
                                                    />
                                                </FormControl>
                                            </FormItem>
                                        )}
