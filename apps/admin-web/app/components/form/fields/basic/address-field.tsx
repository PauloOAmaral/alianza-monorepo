...
import { FormControl, FormError, FormField, FormItem, FormLabel } from '@alianza/ui/form-control'
...
export function AddressField<TFieldValues extends FieldValues>({ control, name, defaultValue, disabled }: AddressFieldProps<TFieldValues>) {
    const { t } = useTranslation()
    const { setValue, resetField, watch } = useFormContext<TFieldValues>()

    const fieldPath = (child: keyof AddressValue) => (name ? `${name}.${String(child)}` : String(child)) as Path<TFieldValues>
        ...
        ? [
              {
                  ...
                  place: [defaultValue.addressLine1, defaultValue.suburb, defaultValue.city, defaultValue.state, defaultValue.country].filter(Boolean).join(', ')
              }
          ]
        : undefined
            ...
            {mode === 'auto' ? (
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <span className='text-xs text-muted-foreground'>
                                <Trans
                                    components={{
                                        button: <Button className='text-xs' isDisabled={disabled} onPress={() => setValue(fieldPath('mode'), 'manual' as any)} variant='link' />
                                    }}
                ...
                <div className='p-4 border border-border rounded-md space-y-4'>
                    <span className='text-xs text-muted-foreground mb-4 block'>
                        <Trans
                            components={{
                                button: <Button className='text-xs' isDisabled={disabled} onPress={() => setValue(fieldPath('mode'), 'auto' as any)} variant='link' />
                            }}
