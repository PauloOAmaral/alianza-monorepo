...
export const TeacherAssignmentExpiredHtml = ({ analystFirstName, teacherName, studentName, expiredAt }: TeacherAssignmentExpiredProps) => {
    const previewText = `Atribuição expirada: ${studentName} → ${teacherName}`

    return (
        <Email previewText={previewText}>
            <Section>
                <Heading className='text-black text-[24px] font-normal text-center p-0 my-[30px] mx-0'>⏱ Atribuição Expirada</Heading>
                <Text className='text-black text-[14px] leading-[24px]'>Olá {analystFirstName},</Text>
                <Text className='text-black text-[14px] leading-[24px]'>
                    A atribuição do aluno <strong>{studentName}</strong> para o professor <strong>{teacherName}</strong> expirou por falta de resposta.
                </Text>

                <Section className='bg-[#fff3cd] border-l-4 border-[#ffc107] rounded-md p-[20px] my-[24px]'>
                    <Text className='text-[#856404] text-[14px] leading-[20px] m-0 font-semibold'>Status: Atribuição expirada (60 minutos sem resposta)</Text>
                    <Text className='text-[#856404] text-[12px] leading-[18px] m-0 mt-[8px]'>Expirado em: {expiredAt}</Text>
                </Section>
...
export const TeacherAssignmentExpiredText = ({ analystFirstName, teacherName, studentName, expiredAt }: TeacherAssignmentExpiredProps) => {
