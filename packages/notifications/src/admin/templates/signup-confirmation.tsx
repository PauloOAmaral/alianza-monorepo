...
export const SignupConfirmationHtml = ({ firstName, lastName, token }: SignupConfirmationProps) => {
    ...
    return (
        <Email previewText={previewText}>
            <Section>
                <Heading className='text-black text-[24px] font-normal text-center p-0 my-[30px] mx-0'>Alianza</Heading>
                <Text className='text-black text-[14px] leading-[24px]'>Olá, {fullName},</Text>
                <Text className='text-black text-[14px] leading-[24px]'>Bem-vindo ao sistema administrativo. Para ativar sua conta, confirme seu email clicando no botão abaixo:</Text>
                <Section className='text-center'>
                    <Button className='bg-[#1d4281] text-white border-none rounded-md px-4 py-2 mx-auto' href={`${baseUrl}/confirm-signup/${token}`}>
                        Confirmar email
                    </Button>
                </Section>
                <Text className='text-[14px] leading-[24px] text-[#6b7280]'>Se você não criou uma conta, pode ignorar este email.</Text>
            </Section>
        </Email>
    )
}
