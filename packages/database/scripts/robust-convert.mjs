...
function convertLine(line, context) {
    ...
    if (context.inColumns && line.includes(':')) {
        // Extract the full line pattern: propertyName: type("DBName", ...)...
        let converted = line
    ...
    if (line.match(/\s*(index|uniqueIndex)\(/)) {
        ...
        // Convert table.propertyName references
        converted = converted.replace(/table\.(\w+)/g, (match, prop) => {
    ...
    // Convert foreign key columns and references
    if (line.includes('columns:') || line.includes('foreignColumns:')) {
        let converted = line.replace(/table\.(\w+)/g, (match, prop) => {
            return `table.${toSnakeCase(prop)}`
        })
...
// Main
console.log('🚀 Starting robust schema conversion...\n')
