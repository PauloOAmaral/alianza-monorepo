        ...
        <div className='border border-border rounded-lg p-2 flex flex-col' key={file.name}>
            <div className='flex items-center gap-2'>
                <div className='w-18 h-14 bg-muted rounded-sm flex items-center justify-center self-start row-span-2 overflow-hidden'>
                    <img alt={file.name} className='w-full h-full object-cover' src={imageUrl} />
                </div>
                ...
                <div className='flex-1 pr-1'>
                    <div className='flex justify-between items-center'>
                        <div className='flex items-center gap-2'>
                            ...
                            <span className='text-sm text-muted-foreground whitespace-nowrap'>{Math.round(file.size / 1024)} KB</span>
                        </div>
                        <Button className='h-8 w-8 bg-transparent! hover:text-red-500' onClick={() => onRemove(file.name)} size='sm' variant='ghost'>
