...
export const getUserContextOptionsForSellerQuery = createAction().build(async () => {
    const db = createMainDbClient()

    const existingSellerContextIds = await db.select({ userContextId: sellers.userContextId }).from(sellers).where(isNull(sellers.deletedAt))
...
export type GetUserContextOptionsForSellerQuery = Awaited<ReturnType<typeof getUserContextOptionsForSellerQuery>>
